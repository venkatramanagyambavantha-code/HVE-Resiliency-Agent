...
* `${input:topology}`: (Required) Exactly `active-active` or `active-standby`.
* `${input:primaryRegion}`: (Required) The current production region.
* `${input:secondaryRegion}`: (Required) The target peer region.
...
3. `primaryRegion` or `secondaryRegion` absent: `primaryRegion and secondaryRegion are required`.
4. `primaryRegion` and `secondaryRegion` resolve to the same normalized region: `primaryRegion and secondaryRegion must differ`.
5. `researchRoot` supplied but not a workspace-relative path, or containing traversal segments or alternate separators: `researchRoot must be a workspace-relative path inside the workspace`.
...
3. For `replaced`, an explicit warning that prior artifacts are invalid.
4. The resolved research root, so the operator can pass it to subsequent steps.
