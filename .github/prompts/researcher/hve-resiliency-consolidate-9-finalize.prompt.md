...
7. Update the Assessment Scope header in its own separate edit: set `Consolidation Status`, `Status Reasons`, `Coverage`, and `Processing Metrics` (candidates, accepted, normalized, rendered, duplicates, citation results, conflicts, hard-limit state). In the same edit, stamp the resolved deployment topology carried verbatim from the frozen manifest: `topology: <active-active|active-standby>` in the document front matter, and `Deployment Topology` in the Assessment Scope block as a stated evaluation condition alongside the resolved regions. Stamp the resolved deployment topology only; never stamp a data write model in that field, and never re-resolve, re-derive, or override the manifest's value.
...
The finalized document carries the manifest's deployment topology in both stamp positions, the front matter and the Assessment Scope block. A missing stamp, or a stamp differing from the manifest, is a named defect for the single permitted corrective render. Stamping adds no section.

## Completion

Report the terminal status with reasons, the stamped deployment topology, coverage and processing metrics, the reconciled finding count, and the Section 9 entry count. End with a next-step suggestion to run `/hve-resiliency-planner-0`.
