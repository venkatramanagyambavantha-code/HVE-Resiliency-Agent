...
argument-hint: "(no required arguments; optional: researchRoot=...)"
---
...
* ${input:researchRoot}: (Optional) Workspace-relative research root. Defaults to `<researchRoot>` when unset, empty, or whitespace. Must resolve to an existing directory inside the workspace.

## Scope and Override

Consolidate completed evidence-only research for the current repository into one authoritative artifact at `<researchRoot>/YYYY-MM-DD-<repo-name>-research.md`. Assess only regional failover between {primaryRegion} and {secondaryRegion}.
...
* Regions Evaluated: {primaryRegion} and {secondaryRegion}
