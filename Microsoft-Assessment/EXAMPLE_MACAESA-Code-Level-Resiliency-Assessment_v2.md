...
4. **Identity and authentication gaps amplify when a second region is added.** The backend Container App enforces no auth at ingress, an all-zeros `sample_user` principal is returned when the auth header is absent, the MCP server runs unauthenticated when JWKS env vars are unset, and the WebSocket endpoint accepts a `user_id` query parameter and uses it as the session key without validating the principal. Adding a second region without fixing these doubles the public attack surface and breaks per-tenant isolation.

5. **External coupling and IaC hardcoding break region symmetry.** Container Apps and the Web App pull a base image from `biabcontainerreg.azurecr.io` (a Microsoft-owned shared registry not under customer control), the Bicep includes hard-coded paired-region tables for Cosmos and Log Analytics, the jumpbox admin password has a literal fallback, and image references use the mutable `latest_v4` tag. Customer-owned mirroring, parameterized region inputs, secret-store-sourced credentials, and digest-pinned images are required before the deployment can be reproduced symmetrically across two regions.

## Themes at a Glance

| # | Theme | Findings |
| --- | --- | --- |
| 1 | No global load balancer or truthful health signal | [P0-021](#p0-021-healthz-returns-healthy-regardless-of-dependency-state), [P0-024](#p0-024-no-glb--front-door--app-gateway--traffic-manager), [P0-025](#p0-025-healthz-registered-with-checks-implementation-evidence-cross-ref-p0-021), [P1-046](#p1-046-app-service-publicnetworkaccess-enabled) |
| 2 | Stateful orchestration lives in process memory | [P0-008](#p0-008-orchestration-uses-inmemorycheckpointstorage), [P0-009](#p0-009-pre-approval-mplan-held-in-asyncioevent), [P0-010](#p0-010-websocket-connection-map-is-in-memory), [P0-011](#p0-011-backgroundtasks-orphans-in_progress-plans), [P0-018](#p0-018-websocket-disconnect--replica-failover-f12), [P0-019](#p0-019-pod-restart-mid-orchestration-f13), [P0-020](#p0-020-approval--clarification-404-after-restart-f14) |
| 3 | Every backing service is single-region by default | [P0-001](#p0-001-foundry--ai-services-account-is-single-region) to [P0-007](#p0-007-enableredundancyfalse-default), [P0-013](#p0-013-cosmos-serverless-cannot-be-promoted-in-place), [P0-014](#p0-014-cosmos-unavailable--regional-outage-unhandled-f1), [P0-030](#p0-030-cosmos-pe-only-in-primary-while-cosmos-fails-over), [P1-001](#p1-001-azure-openai-deployments-single-region) to [P1-006](#p1-006-storage-blob-default-replication-lrs), [P1-017](#p1-017-backend-env-hardcodes-single-region-urls) |
| 4 | Identity and authentication gaps amplify with a second region | [P0-026](#p0-026-backend-has-no-auth-enforcement-at-container-app-ingress), [P0-027](#p0-027-sample_user-all-zeros-principal-fallback), [P0-028](#p0-028-mcp-server-runs-unauthenticated-when-enable_authtrue-but-jwksissueraudience-env-vars-are-unset), [P0-029](#p0-029-websocket-user_id-accepted-from-query-param-without-token-validation), [P1-010](#p1-010-app-service-easy-auth-single-tenant-config), [P1-011](#p1-011-mcp-jwtjwks-single-region-issuer-config), [P1-042](#p1-042-defaultazurecredential--mic-without-tenant_id), [P1-043](#p1-043-no-retry--backoff--fallback-on-get_access_token), [P1-044](#p1-044-single-uami-sole-rbac-subject-across-data-planes), [P1-045](#p1-045-container-apps-ingressexternal-true) |
| 5 | External coupling and IaC hardcoding break region symmetry | [P0-005](#p0-005-single-bicep-location-parameter), [P0-022](#p0-022-external-acr-biabcontainerregazurecrio), [P0-023](#p0-023-jumpbox-admin-password-fallback-literal), [P1-012](#p1-012-external-biabcontainerregazurecrio-shared-registry), [P1-013](#p1-013-hard-coded-cosmos-region-pair-map), [P1-014](#p1-014-hard-coded-log-analytics-replica-region-pairs), [P3-009](#p3-009-container-images-use-mutable-latest_v4-tag) |
...
* **Cross-refs:** P0-002 (Project + Agent Service replication), P0-006 (OpenAI deployments), P0-016 (5xx-aware retry path), P0-017 (Foundry health probe).
* **Implementation:** The `binggrnd` connection name stays identical across regions; only the underlying API key / managed-identity binding is re-issued to the peer Project.
...
* **Implementation:** Treat `data/agent_teams/*.json` as the seed catalog.
* **Validation:** Replay the seed against both Projects and assert `count(agents)` matches; chaos-stop the primary Foundry account and submit a plan via `{secondaryRegion}`. Agent-count drift surfaces as `agents.drift=true` and the probe returns 503.
...
* **Cross-refs:** P0-013 (serverless cannot be promoted in place; prerequisite migration), P0-014 (client-side retry / failover), P0-015 (429 handling), P0-030 (PE in the secondary VNet).
* **Validation:** If only the *peer* is reachable, return 200 with `cosmos.local=false, cosmos.peer=true` (degraded-but-serving) so GLB keeps the region in rotation.
...
* **Cross-refs:** P0-005 (per-region location parameters), P0-030 (Cosmos PE in the secondary VNet).
* **Validation:** From each region, `nslookup` must resolve to the *local* PE private IP, not the peer region's range.
...
* **Cross-refs:** Replaces the static `cosmosRegionPair` / `logAnalyticsRegionPair` lookup tables (P1-013, P1-014); prerequisite for P0-001, P0-003, P0-004, P0-006, P0-007.
* **Implementation:** Surface the new parameter in `azure.yaml` / `azure_custom.yaml`.
...
* **Cross-refs:** P0-001 (resolver), P0-016 (5xx + failover at the call site).
* **Validation:** From each region's backend replica, issue a `chat.completions` call and assert the response telemetry `azure.region` matches the local region.
...
* **Cross-refs:** P1-009 (jumpbox PPG / single-zone pinning), P0-021 (ready-probe contract).
* **Implementation:** Drop the proximity placement group requirement on the jumpbox so it is not pinned to a single zone.
* **Validation:** A region is ready when at least 2 of 3 zones report healthy replicas; single-zone loss must not flip the region unhealthy.
...
* **Cross-refs:** P0-011 (durable runner), P0-019 (startup recovery scan).
* **Implementation:** Add a `checkpoints` container to `infra/main.bicep` partitioned on `/plan_id`. Gate `/healthz/ready` on `recovery.complete=true` so GLB does not route fresh plan starts into a replica that has not finished sweeping orphans.
...
* **Cross-refs:** P0-012 (idempotency keys), P0-020 (stateless approval routes), P0-010 (backplane).
* **Implementation:** Change-feed-driven notification removes polling cost once the P0-010 backplane is in place.
...
* **Cross-refs:** P0-018 (replay-on-reconnect), P0-029 (token-derived `user_id`).
* **Implementation:** Provision Service Bus Premium per region with geo-DR pairing; grant the workload UAMI `Azure Service Bus Data Sender` and `Receiver`.
* **Guardrail:** CI must grep Bicep for `Endpoint=sb://`, `SharedAccessKey=`, `AccessKey=` and fail on match.
...
* **Cross-refs:** P0-008, P0-019.
* **Implementation:** Use `DefaultAzureCredential` to authenticate to Service Bus. Gate `/healthz/ready` on `app.state.recovery_complete=true`.
* **Guardrail:** CI must reject any `SharedAccessKey=` literal in Bicep or Container Apps `secrets[]`.
...
* **Implementation:** Frontend must generate a stable UUID per user action and resend the same key on retry. Centralize the dependency so future mutating routes inherit the discipline. Do not log the idempotency key alongside user identifiers.
...
* **Cross-refs:** P0-003, P0-014, P0-015, P0-030.
* **Validation:** End-to-end smoke: write to `{primaryRegion}` and read from `{secondaryRegion}` within 5s under session consistency.
* **Guardrail:** Pipeline fails if `listKeys(cosmos` appears in any deployed Bicep output.
...
* **Cross-refs:** P0-003 (multi-region account; prerequisite), P0-015 (429 handling in a sibling helper), P0-021/P0-025 (Cosmos probe registration).
* **Validation:** Chaos test: block egress to the primary Cosmos FQDN and assert reads succeed against `{secondaryRegion}` within the SDK timeout window.
...
* **Cross-refs:** P0-014 (sibling 5xx path), P1-022 (separation of 429 from generic exceptions).
* **Validation:** Load test: ramp to 2× provisioned RU; assert API stays under 1% 5xx and `cosmos.throttled` rises while end-user requests still succeed. Budget test: unbounded 429 mocks must raise after the 10s deadline (no infinite retry).
...
* **Cross-refs:** P0-001 (`resolve_foundry_endpoint`, `peer_foundry_endpoint`, `mark_foundry_unhealthy`), P0-017 (Foundry probe feeds health).
* **Validation:** A region stays ready when *either* local or peer Foundry is reachable; only dual-failure flips it 503.
* **Guardrail:** Build gate fails if `AZURE_OPENAI_API_KEY` appears in any `secrets[]`.
...
* **Cross-refs:** P0-001 (resolver + health flags), P0-016 (call-site retry that consults the same flag), P0-021/P0-025 (probe registry that publishes the readiness payload).
* **Validation:** Recovery test: bring the endpoint back and assert `foundry.local=true` flips after 60s of consecutive successes (no flap).
...
* **Cross-refs:** P0-010 (backplane), P0-029 (WS auth).
* **Implementation:** Backplane subscription TTL ≥ 24h so reconnects within the window do not drop messages.
...
* **Cross-refs:** P0-008 (checkpoint store), P0-011 (durable runner), P0-021 (readiness gate).
* **Implementation:** Stale-lease threshold (60s) must exceed the worker heartbeat interval to avoid double-leases.
...
* **Cross-refs:** P0-009, P0-011, P0-012.
* **Implementation:** Orchestration loop wakes via the approvals change feed (or next poll) and resumes the plan from the latest checkpoint.
...
* **Cross-refs:** P0-026 (probe path is exempt from auth).
* **Implementation:** Foundational finding: every dependency-bearing P0/P1 (Cosmos, Foundry, ACR, Service Bus, MCP, auth) registers a probe in this registry. GLB probe contract: `GET /healthz/ready`, HTTPS, 5s interval, 2s timeout, 3 successes = healthy, 2 failures = unhealthy. On cross-region degradation, when the local dependency is down but the peer is reachable, return 200 with `<probe>.local=false, <probe>.peer=true`.
...
* **Implementation:** Pin to immutable digests (not floating tags) once mirrored. ACR is non-critical for live requests (image is cached on running replicas), so it surfaces in `/healthz` as `acr.local_replica=ok|fail` but does not flip readiness.
* **Guardrail:** Grep `infra/**` and `src/**` for `biabcontainerreg.azurecr.io` and fail on any match.
...
* **Implementation:** Never grant the workload UAMI access to this secret; only the deployer principal needs `Key Vault Secret User`.
* **Validation:** `bicep build` produces a JSON artifact with no literal; `git grep` returns no matches; `azd provision` without override fails with a clear "parameter required" error.
...
* **Cross-refs:** P0-026 (probe path bypasses auth), P0-021 (readiness contract this GLB layer consumes).
* **Implementation:** `/healthz/ready` is allow-listed in Easy Auth `excludedPaths` so anonymous probes are not rejected. Origin lock-down: the backend rejects requests whose `X-Azure-FDID` header does not match the configured Front Door instance ID.
...
* **Cross-refs:** P0-021 (inherits the GLB readiness contract), P3-010.
* **Guardrail:** Build-time grep must reject any reintroduction of `checks={}` or `password=""`.
...
* **Cross-refs:** P0-029 (apply the same validator on WebSocket upgrade), P0-027 (delete the `sample_user` fallback in the same change), P1-045.
* **Implementation:** Audience and issuer are sourced from Bicep params; no client IDs or secrets in code. Register `AuthMiddleware` after `HealthCheckMiddleware` so probes are reached first.
...
* **Cross-refs:** P0-026 (`AuthMiddleware` returns 401 on a missing principal instead of the deleted fallback), P0-029 (WebSocket auth uses the validated `oid`).
* **Validation:** Quarantine query `SELECT VALUE COUNT(1) FROM c WHERE c.user_id = '00000000-...'` should return 0 in production.
...
* **Cross-refs:** P0-021 (MCP probe registry; a misconfigured region's revision never reaches `Running`, so the backend MCP probe fails and `/healthz/ready` returns 503), P1-011, P3-014.
* **Implementation:** Wire `JWKS_URI`, `MCP_ISSUER`, `MCP_AUDIENCE` from Bicep params/Key Vault into both regional MCP Container Apps.
* **Guardrail:** CI test must boot the image with missing env and assert a non-zero exit.
...
* **Cross-refs:** P0-026/P0-028 (shared JWKS dependency; JWKS unreachable flips `/healthz/ready` to 503 and the GLB drains the region for HTTP and WebSocket traffic).
* **Implementation:** JWKS URI must be sourced from `app_config.jwks_uri`, never hard-coded.
* **Guardrail:** Regression grep must fail the build on `user_id = websocket.query_params`.
...
* **Cross-refs:** P0-003 (multi-region Cosmos), P0-004 (per-region VNet fabric).
* **Implementation:** Cosmos `publicNetworkAccess` must remain `Disabled`; no IP allow-list workarounds.
* **Validation:** Trigger a Cosmos manual failover and confirm the SDK in `{secondaryRegion}` continues reads/writes via the local PE.
...
* **Cross-refs:** P0-001 (peer Foundry account), P1-017 (env-driven endpoint resolution).
* **Validation:** Pre-validate quota with `az cognitiveservices usage list` per region before flipping deployments.
...
* **Cross-refs:** P1-016 (replicas/partitions/public-access), P1-017 (endpoint resolution), P0-021 (readiness contract).
* **Validation:** Run an index parity check after deploy: `count(documents)` matches between regions within tolerance.
...
* **Cross-refs:** P0-004 (regional VNets), P0-021 (readiness contract).
* **Validation:** Drain test: deploy a new revision at `weight=0`, ramp to 100 over 60s, confirm zero failed in-flight plans via Application Insights `requests`.
...
* **Cross-refs:** P0-024 (GLB / Front Door fronting both origins), P0-021 (readiness contract), P1-010 (Easy Auth symmetry).
* **Implementation:** Mirror the plan + Web App for `{secondaryRegion}`.
...
* **Cross-refs:** P1-019 (regional login servers in image references), P0-022 (mirror upstream registry into customer-owned ACR).
* **Validation:** Verify `az acr replication list -r <acr>` shows both replications in `Succeeded`.
...
* **Cross-refs:** P1-020 (the threading bug itself), P1-017 (env-driven endpoint resolution).
* **Validation:** `az storage account show` returns `sku.name=Standard_GZRS` (or RAGZRS) post-deploy.
...
* **Cross-refs:** P1-042, P1-044.
* **Implementation:** Build `DefaultAzureCredential(managed_identity_client_id=os.environ["AZURE_CLIENT_ID"])` in `app_config` so each replica binds deterministically to its local UAMI's client id.
...
* **Cross-refs:** P1-009 (jumpbox VMSS), P1-015 (combined operator-path zone gap).
* **Implementation:** Bastion Basic SKU does not support zones; Standard is required.
...
* **Cross-refs:** P1-008 (Bastion zones), P1-015 (combined operator path), P0-023 (`adminPassword` from Key Vault).
* **Validation:** With Chaos Studio, stop one zone; VMSS keeps `capacity=1` by reallocating into a healthy zone within minutes.
...
* **Cross-refs:** P0-026, P1-046.
* **Implementation:** Configure `excludedPaths: ['/healthz/*']` so GLB probes bypass auth.
...
* **Cross-refs:** P1-040 (surface validator state in the MCP `/healthz/ready` probe), P0-028, P1-038.
* **Implementation:** Surface validator state in the MCP `/healthz/ready` probe.
...
* **Cross-refs:** P0-022, P1-005, P1-019, P1-038.
* **Implementation:** Set `tagImmutability: 'Enabled'` on the customer ACR.
* **Validation:** Verify Notation signatures at admission via Container Apps registry image verification or Defender for Containers policy.
...
* **Cross-refs:** P0-005 (parameter shape), P0-003 / P0-013 (Cosmos consumption).
* **Implementation:** Add a Bicep `assert` that `primaryLocation != secondaryLocation` plus same-geography enforcement where data residency requires it. Update `infra/main.parameters.json` to surface `secondaryLocation`.
...
* **Cross-refs:** P0-005, P1-013 (shared parameter).
* **Validation:** Confirm ingestion from both regions via `Heartbeat | summarize count() by _ResourceId, ResourceLocation` after deploy.
...
* **Cross-refs:** P1-008, P1-009, P0-023 (jumpbox `adminPassword` from Key Vault).
* **Validation:** With combined zone-1 chaos, the operator must still RDP through Bastion to a jumpbox instance in zone 2 or 3.
...
* **Cross-refs:** P1-002 (multi-region provisioning), P1-017 (endpoint resolution), P0-021 (readiness contract).
* **Validation:** Public-internet `curl` to the Search FQDN returns DNS NXDOMAIN or 403 post-deploy.
...
* **Cross-refs:** P0-001 (Foundry pair), P1-002 / P1-016 (Search pair), P1-006 (Storage redundancy), P1-018 (Cosmos `preferred_locations`).
* **Implementation:** Mirror the env block for `mcp_server`. Add `resolve_search_endpoint()` and peers in `src/backend/common/config/app_config.py` keyed on `APP_REGION`.
...
* **Cross-refs:** P0-014 (retry policy), P1-024 (singleton wrapper), P2-010 (Session consistency).
* **Validation:** `client.client_connection.ReadEndpoints` shows the local region first from each replica.
...
* **Cross-refs:** P1-005 (ACR geo-replication), P0-022 (mirror upstream registry), P0-021 (readiness depends on a successful pull).
* **Validation:** `az containerapp show --query 'properties.template.containers[].image'` returns digests, not floating tags, in both regions.
...
* **Cross-refs:** P1-006 (full SKU + redundancy guidance), P0-021 (readiness contract).
* **Validation:** Bicep `what-if` confirms the SKU is pinned by the deploy, not inherited from AVM defaults.
...
* **Cross-refs:** P1-023 (pre-condition for safely enabling multi-region writes).
* **Validation:** Sustained `ConcurrentUpdateError` rate above threshold should mark the region degraded so GLB de-weights writes.
...
* **Cross-refs:** P0-014, P0-015, P0-021.
* **Validation:** Sustained `CosmosDependencyDegraded` for >10s on the local region must surface in `/healthz/ready` so GLB de-weights the region.
...
* **Cross-refs:** P0-013 (off serverless), P0-003 (locations), P1-021 (etag), P1-018 (preferred_locations) - all pre-conditions.
* **Validation:** Validate dual-write convergence and run a `failover-priority-change` drill.
...
* **Cross-refs:** P1-018 (SDK flag), P1-026 (health-driven invalidation), P1-017 (region env).
* **Implementation:** Pair with P1-026 so the singleton is replaced (not patched) when the resolved endpoint becomes unhealthy.
...
* **Cross-refs:** P1-027 (WebSocket retry), P0-018 (reconnection) so drained sockets reconnect to the new revision or peer region.
* **Implementation:** Pair with a SIGTERM handler in `src/backend/v4/api/lifespan.py` that sets `app.state.ready = False` and a revision-ramp script that uses `az containerapp ingress traffic set` to ramp 0→100.
...
* **Cross-refs:** P0-021.
* **Validation:** `/healthz` exposes `cosmos.consecutive_failures` and `cosmos.last_invalidated_at`; sustained `cosmos.healthy=false` for >10s flips `/healthz/ready` to 503.
...
* **Cross-refs:** P0-018, P1-025, P1-028.
* **Implementation:** Emit `ws.send.retry`, `ws.send.deadletter`, `ws.ack.timeout` metrics.
* **Validation:** Sustained `ws.send.deadletter` rate above threshold should mark the region degraded so GLB de-weights it.
...
* **Cross-refs:** P0-018 (pub/sub backplane so a peer region can replay the buffer), P0-021 (readiness write probe).
* **Implementation:** `/healthz/ready` should include a write probe to `agent_stream_events`.
...
* **Cross-refs:** P0-030 (Private DNS), P0-021.
* **Implementation:** Mirror the PE in IaC at `infra/main.bicep:1088-1101`.
* **Validation:** NSG-block the primary PE NIC and observe `/healthz/ready` flip to 503 within 10s.
...
* **Cross-refs:** P1-001 (peer Foundry deployment), P0-021.
* **Validation:** Sustained `openai.429.deferred` above threshold for 60s marks the region degraded.
...
* **Cross-refs:** P1-002 (peer Search), P0-021.
* **Validation:** `/healthz/ready` returns 503 only when both local and peer Search are unavailable.
...
* **Cross-refs:** P1-006 (GZRS), P0-021.
* **Validation:** `/healthz/ready` returns 503 only when both local and `-secondary` blob HEADs fail.
...
* **Cross-refs:** P1-005, P1-019, P0-022.
* **Validation:** Block egress to the primary ACR and confirm new revisions still pull within 60s.
...
* **Cross-refs:** P1-042 (explicit `tenant_id`), P0-021.
* **Implementation:** Token cache must be in-memory only (no disk persistence).
...
* **Cross-refs:** P0-021.
* **Implementation:** The Web App's `/healthz/ready` should probe the local backend `/healthz/ready` over the same client with a 2s timeout.
...
* **Cross-refs:** P1-005 / P1-019 (private registry inside the customer-owned ACR).
* **Implementation:** Stand up the private registry inside the customer-owned ACR.
* **Guardrail:** CI gate runs `az deployment sub what-if` against both `{primaryRegion}` and `{secondaryRegion}` and fails on drift before any AVM bump.
...
* **Cross-refs:** P1-019.
* **Implementation:** Pair with a Dockerfile that sets `PIP_INDEX_URL` to the private feed and runs `pip install --require-hashes -r requirements.lock`. Track upstream stable through a controlled bump PR.
...
* **Cross-refs:** P0-022, P1-011, P1-017, P1-040.
* **Implementation:** Add `app_config.resolve_mcp_endpoint(peer=False)` returning the local URL based on `APP_REGION`; on local failure, return the peer URL with an `mcp.fallback` metric. Front both origins with the GLB.
...
* **Cross-refs:** P1-007, P1-044.
* **Implementation:** Snapshot customer-authored teams nightly to blob with UAMI auth (no SAS).
* **Guardrail:** CI drift check diffs `team_ids` between regions daily.
...
* **Cross-refs:** P0-021, P0-024.
* **Implementation:** Mirror in MCP: register `@app.get("/healthz")` and `/healthz/ready` (probes the JWKS cache), and add `HEALTHCHECK CMD curl -f http://localhost:8000/healthz || exit 1` to the MCP Dockerfile. Configure GLB probes against `/healthz/ready` at `interval=5s, timeout=2s`.
...
* **Cross-refs:** P1-030 (OpenAI), P0-021.
* **Validation:** Sustained `<dependency>.retry.exhausted > threshold` for 60s flips `/healthz/ready` to 503.
...
* **Cross-refs:** P1-007, P1-043.
* **Implementation:** Add a FastAPI middleware that decodes `X-MS-CLIENT-PRINCIPAL` and rejects 401 on `tid` mismatch. Surface `auth.tenant_validated` in `/healthz`.
...
* **Cross-refs:** P1-034, P1-042.
* **Implementation:** Surface `auth.last_retry_count` and `auth.fallback_used`. Token cache must be in-memory only, never on disk.
...
* **Cross-refs:** P1-007 (mirror per-region).
* **Implementation:** Add `data_plane_self_test` in `/healthz` so per-plane health is visible. Mirror per-region. No subscription-scope `Contributor` assignments.
...
* **Cross-refs:** P0-026, P2-017, P1-046.
* **Implementation:** Mirror the gate for the MCP Container App.
...
* **Cross-refs:** P1-010 (Easy Auth enforcement), P0-024, P1-004, P1-045.
* **Implementation:** Mirror for the `{secondaryRegion}` Web App.
...
* **Cross-refs:** P2-011.
* **Implementation:** Pair with P2-011 so the runtime treats a missing or invalid connection string as a degraded-mode warning rather than an unhandled silent drop.
...
* **Implementation:** The operator override unblocks deployments outside the canonical paired-region table and removes a silent IaC dependency on the hard-coded map.
...
**Impact:** Operators selecting any region outside the hardcoded 10-entry table see HA-related fields silently resolve to `null`, with no clear failure signal in the deployment log. This is a code-quality / IaC hygiene concern that complicates onboarding new regions.
...
* **Cross-refs:** P2-004, P2-007 (Cosmos consistency / backup hygiene).
* **Implementation:** Pairs the deploy-time validation with an operator escape hatch, aligning with the parameter-validation pattern used elsewhere in the template.
...
* **Cross-refs:** P2-010 (pins the same level on the application `CosmosClient`).
...
* **Validation:** Record `x-ms-request-charge` per call in tests and assert below an agreed RU threshold to prevent regressions.
...
* **Validation:** Run `pytest` with `asyncio_mode=auto`, trigger lifespan shutdown, and assert `close_all` was awaited.
...
**Issue:** The Cosmos AVM module call does not pass `backupPolicyType` or related fields, so the account inherits the default 4-hour periodic policy. Backup posture is identical regardless of region count, and serverless mode also blocks continuous backup (PITR). The gap is data-protection best practice.
...
* **Validation:** `az cosmosdb show --query backupPolicy` after deploy.
...
**Issue:** The Storage `blobServices` block sets only soft-delete and access tracking; `isVersioningEnabled`, `restorePolicy`, and `changeFeed` are not configured. Blob protection posture is identical in single-region and active/active. The gap is data-protection hygiene.
...
* **Validation:** Confirm with `az storage account blob-service-properties show` after deploy.
...
**Issue:** All `az storage blob upload-batch` calls use `--overwrite` with no ETag or `If-None-Match` guard, so a second operator running the same script clobbers the first one's uploads. Bulk-upload script behaviour is identical regardless of region count. The gap is operator-script idempotency hygiene.
...
* **Validation:** Run two operator scripts concurrently and confirm the second exits cleanly with a lease error.
...
* **Cross-refs:** P2-004 (IaC pin).
* **Validation:** Integration test for read-your-writes within the same session.
...
**Issue:** `azure-monitor-opentelemetry` configuration and `track_event` calls return without raising on exporter failure, so live metrics blackouts during incidents look identical to a healthy boot. Silent-drop behaviour is the same in single-region and active/active topologies. The gap is observability hygiene around exporter health.
...
* **Cross-refs:** P2-001 (component provisioning), P2-022 (structured logging depth).
...
**Impact:** Two components calling the same SDK with different versions can exhibit subtly different retry, token-refresh, or failover behavior, complicating diagnosis. Beta pins also expose the build to deletion / yank by upstream maintainers. This is a dependency hygiene concern.
...
* **Cross-refs:** P3-007 (private wheel mirror), P2-013 (CI/CD supply-chain hygiene).
* **Implementation:** Apply the same `-c ../../constraints.txt` reference in `src/mcp_server/requirements.txt` and operator scripts under `infra/scripts/`.
...
**Issue:** Every reusable GitHub Action in `.github/workflows/` is referenced by a moving tag (for example `actions/checkout@v4`, `Azure/setup-azd@v2`, `azure/login@v2`) rather than by an immutable commit SHA. This behaves identically in single-region and active/active deployments; tag mutability is a CI/CD concern. A compromised or retagged action could ship arbitrary code into the next build without any code change in this repository.
...
**Impact:** A tag-rewrite or upstream compromise of any pinned action propagates immediately into the next workflow run, including DR re-deploys. This is a supply-chain hygiene concern.
...
* **Cross-refs:** P2-012 (Python SDK pins), P3-007 (private wheel mirror) - both contribute to a reproducible-build supply chain.
* **Implementation:** Apply across every file under `.github/workflows/`.
...
**Impact:** Operators integrating with an existing hub network must edit the template to avoid CIDR collisions. The oversized `/8` default also wastes a top-level RFC1918 block. This is an IaC parameterization / hygiene concern.
...
* **Cross-refs:** P2-021 (route tables / NAT / DDoS, where the same subnet plan is consumed).
* **Implementation:** Thread the same `subnetPrefixes` object into `infra/modules/virtualNetwork.bicep` so the module references the parameter rather than its own hardcoded array.
...
**Impact:** A future contributor wiring a new SDK client through `ManagedIdentityCredential` may pick up the SAMI principal id and silently get 403s, with no obvious link back to the IaC identity-model split. This is an identity-model consistency / hygiene concern.
...
* **Cross-refs:** P2-016 (RBAC propagation gating; a single principal id makes the propagation probe trivial).
* **Implementation:** The same UAMI is already used by the AI Foundry, Cosmos, Search, and OpenAI role assignments, so this aligns Web Site and Storage with the rest of the stack.
...
**Impact:** Cold deployments and `azd up` runs can mark the deployment "succeeded" while the workload's first requests fail with `AuthorizationFailed`, complicating triage and CI smoke checks. This is a deploy-time hygiene concern.
...
* **Cross-refs:** P2-015 (UAMI standardization).
* **Validation:** The probe runs from the deployment principal's context, sufficient to confirm the assignment is materialized (also cited as OQ-10 in the master plan).
...
**Impact:** Any browser that sends credentials cross-origin will see the response rejected, while non-credentialed requests from arbitrary origins are accepted. This is a security-posture hygiene concern.
...
* **Cross-refs:** P1-045 (external ingress; both layers need the same allow-list to be effective).
* **Implementation:** The Container App `corsPolicy` in `infra/main.bicep` should mirror this list.
...
**Impact:** Transient identity outages or RBAC-propagation lag (cross-ref P2-016) cause container-app replicas to crash-loop on first start, generating noisy alerts and delaying recovery. This is a startup-robustness hygiene concern.
...
* **Cross-refs:** P2-011 (telemetry exporter silent drop), P2-016 (RBAC propagation gate).
* **Implementation:** Pair with a `/healthz` extension that reports `identity=degraded` until the first `get_token` call succeeds.
...
**Impact:** Operators enabling private networking still get a publicly reachable AI Foundry data plane unless they manually edit the resource post-deploy. This is a network-posture hygiene concern.
...
* **Cross-refs:** P2-020 (Storage `bypass`; stack-wide consistency).
* **Implementation:** Propagate the same pattern to other Cognitive Services accounts in the template.
...
**Impact:** A misconfigured first-party Azure service in the same tenant could reach the Storage account unintentionally, despite public access being disabled. This is a posture-mismatch hygiene concern.
...
* **Cross-refs:** P2-019 (stack-wide ACL consistency).
* **Implementation:** Audit which Azure services genuinely need access and add named exceptions only for those (typically none in this workload).
...
**Impact:** Bursty outbound traffic can hit SNAT exhaustion under load, surfacing as random `ConnectionResetError` against external dependencies, and the workload has no DDoS Protection Standard for production. This is a network-controls hygiene concern.
...
* **Cross-refs:** P2-014 (parameterized CIDRs; the same `vnetAddressPrefix` parameter is reused).
* **Implementation:** Deploy a NAT Gateway per region in active/active so {primaryRegion} and {secondaryRegion} each have their own egress IP.
...
**Issue:** Health probes, correlation IDs, structured logging, dependency-call telemetry, and business metrics are minimal or absent, and some transaction-critical paths still use `print(...)`. Telemetry depth does not change with region count, so silent failures stay silent in both single-region and active/active. The gap is observability hygiene.
...
* **Cross-refs:** P2-011.
* **Implementation:** Pair with P2-011 so exporter failures and structured log records arrive at the same backend. See also the Application Insights `requests` table for dependency `operation_Id` propagation.
...
* **Implementation:** Operator-only telemetry path.
* **Validation:** `az monitor data-collection rule show` in both regions; confirm jumpbox heartbeat ingestion in each workspace.
...
* **Validation:** Grep for `azure.cosmos.CosmosClient` returns zero hits after the change; existing Cosmos integration tests pass without modification.
...
* **Implementation:** Update sibling delete helpers (`delete_data`, etc.) to the same contract. Callers should explicitly handle `NOT_FOUND` without raising.
...
* **Cross-refs:** P0-026 (Easy Auth not provisioned), P0-027 (auth disabled by default).
* **Implementation:** Exclude the `sample_user` module from production images or guard it with `DEV_MODE`.
...
* **Cross-refs:** P0-028 (MCP unauthenticated when JWKS env unset), P3-014.
* **Implementation:** Pair with P3-014 to flip the default-false flag for production.
...
**Impact:** A from-source DR rebuild creates three independent supply-chain dependencies. Hygiene gap.
...
* **Implementation:** Apply uniformly across `src/backend/Dockerfile`, `src/mcp_server/Dockerfile`, and `src/App/Dockerfile`.
* **Validation:** Build with public registries blocked.
...
**Impact:** Operator-script reproducibility in `{secondaryRegion}` is gated on PyPI / npm reachability. Hygiene gap.
...
* **Cross-refs:** P2-012 (SDK pinning alignment).
* **Implementation:** Apply the same pattern to frontend `npm install` (use `--registry $env:INTERNAL_NPM_REGISTRY`).
...
* **Cross-refs:** P3-013 (related tenant-id duplication).
* **Implementation:** No source change; recorded so the global control plane is explicit in the report.
...
* **Implementation:** Digest-pinning is the recommended remediation for image integrity.
* **Validation:** `az containerapp show` confirms a digest-pinned image, and re-deploy with the same digest yields identical revision content.
...
* **Cross-refs:** P0-025 (synthetic OK on `/healthz`).
* **Guardrail:** Grep returns zero hits for empty-string credential literals in middleware constructors.
...
* **Implementation:** Document the segmentation contract alongside the module.
* **Validation:** Per-subnet differentiation with `az network nsg rule list`.
...
* **Implementation:** Provision per region; document retention.
* **Validation:** Confirm flow log blobs land in the configured Storage account post-deploy.
...
* **Cross-refs:** P1-042.
* **Implementation:** Update any caller of `cfg.TENANT_ID` / `cfg.CLIENT_ID` to use the canonical names.
* **Validation:** Confirm both attributes are env-sourced (no literal tenant id in source or `.env.example`).
...
* **Cross-refs:** P0-026 (Easy Auth not provisioned), P0-028 (MCP unauthenticated when JWKS env unset).
* **Implementation:** Production parameter sets must default `true`; dev overrides should remain explicit.
...
The infrastructure is expressed in Bicep with Azure Verified Modules (AVM) under `infra/main.bicep` and `infra/modules/`, plus operator post-deploy PowerShell under `infra/scripts/`. The IaC is single-region by construction: one `location` parameter threads through every module, AVM versions and SKUs are pinned, and feature flags (`enableRedundancy`, `enablePrivateNetworking`, `enableMonitoring`, `enableScalability`) gate optional posture rather than per-region replication. Every multi-region resiliency decision therefore falls outside the repo, onto the operator and platform.

The first table lists what the repo configures today, grouped by domain, and how each choice affects region resiliency. The second lists the capabilities the repo does not provide, grouped by who owns the gap. The Findings column traces each row to its detailed entries in [Section 5](#5-full-finding-matrix).

## Available to Review

| Domain | Configuration | Current value | Gap / risk | Findings |
| --- | --- | --- | --- | --- |
| Region topology & parameters | `location` parameter | Single string parameter, no `primaryLocation` / `secondaryLocation` split | Threaded through every AVM module; the deployment cannot be projected into a second region without forking the parameter set | P0-005, P1-013, P1-014, P1-017 |
| Region topology & parameters | `enableRedundancy` flag | `bool = false` default | Not threaded through every AVM module call (notably Storage); zone redundancy on Container Apps, App Service, Bastion, Cosmos, AI Search is gated off by default | P0-007, P1-003, P1-004, P1-006, P1-008, P1-020 |
| Region topology & parameters | `enablePrivateNetworking` flag | `bool = false` default | Gates the entire VNet / NSG / Private DNS / Private Endpoint / Bastion / jumpbox stack; when false the platform is fully public; when true the PE/DNS topology is single-region only | P0-004, P0-030 |
| Region topology & parameters | `enableMonitoring` flag | `bool = false` default | Gates Application Insights component and Log Analytics workspace creation; when false the platform has no first-class observability | P2-001, P2-002, P3-001 |
| AI Foundry & OpenAI | AI Foundry account (`Microsoft.CognitiveServices/accounts`) | AVM `avm/res/cognitive-services/account` provisioned at `azureAiServiceLocation`; `networkAcls.defaultAction: Allow` | Single-region Foundry control plane; no secondary account in `{secondaryRegion}`; `Allow` default action exposes the data plane to the public Internet | P0-001, P0-002, P0-006, P2-019 |
| AI Foundry & OpenAI | AI Foundry Project + Agent Service | Single project, agent metadata seeded once via deployment script | Agent IDs are not portable to a second project; reseeding required during failover | P0-002, P1-039 |
| AI Foundry & OpenAI | Azure OpenAI deployments (`gpt-4.1-mini`, `gpt-4.1`, `o4-mini`) | Deployed under the single Foundry account at `azureAiServiceLocation` | All inference traffic projects into one region; no PTU pinning, no secondary deployment | P1-001 |
| Cosmos DB | Cosmos DB account | AVM `avm/res/document-db/database-account` with `capabilities: [{ name: 'EnableServerless' }]`; `cosmosDbZoneRedundantHaRegionPairs` map for HA | Serverless cannot be promoted to multi-region writes in place; HA region resolved from a hardcoded 10-row pair table; no secondary write region | P0-003, P0-013, P1-013, P1-023, P2-003, P2-004, P2-007 |
| Cosmos DB | Cosmos Private Endpoint | PE only in primary VNet; Private DNS zone linked to primary VNet only | If Cosmos fails over to its paired region, the PE remains anchored to the primary VNet and DNS resolution breaks for any caller in `{secondaryRegion}` | P0-030, P1-029 |
| Backing services | AI Search service | AVM `avm/res/search/search-service` with `replicaCount: 1`, `partitionCount: 1`, `publicNetworkAccess: 'Enabled'` (gated by `enablePrivateNetworking`) | Below the SLA-bearing 2×3 minimum; single-region; no replica in `{secondaryRegion}`; index rebuild required during failover | P1-002, P1-016 |
| Backing services | Storage account | AVM `avm/res/storage/storage-account` invoked without `skuName`; `enableRedundancy` not threaded; `networkAcls.bypass: 'AzureServices'` | Default LRS; no GZRS / RA-GZRS pin; trusted-services bypass leaves a public ingress path even when private networking is on | P1-006, P1-020, P2-008, P2-020 |
| Backing services | Container Apps Environment | AVM `avm/res/app/managed-environment` with `zoneRedundant: enableRedundancy`; `revisionsMode: 'Single'`; `ingressExternal: true` on Container Apps | Zone redundancy off by default; single-revision mode + external ingress on backend means there is no ingress isolation and no graceful drain on revision swap | P1-003, P1-025, P1-045 |
| Backing services | App Service Plan + Web App | AVM `avm/res/web/serverfarm` + `avm/res/web/site`; `publicNetworkAccess: 'Enabled'`; single region | Frontend Web App is publicly addressable; no per-region App Service Plan; no GLB-fronted dual-region site | P1-004, P1-046 |
| Observability | Application Insights | AVM `avm/res/insights/component` (single component) | One regional component; telemetry from a `{secondaryRegion}` workload would land in the primary-region component | P2-001, P2-011 |
| Observability | Log Analytics workspace | AVM `avm/res/operational-insights/workspace`; `replicaRegionPairs` map for cross-region replica | Workspace is created in primary region; replica region is chosen from a hardcoded 10-row pair table; operator cannot override at deploy time | P1-014, P2-002 |
| Identity, auth & health | User-Assigned Managed Identity + role assignments | Single UAMI `id-${solutionSuffix}`; role assignments scoped to the primary resource group only | Same UAMI is the principal across Cosmos / Foundry / Search / Storage data planes; role assignments do not extend to a `{secondaryRegion}` resource group | P1-007, P1-044, P2-015, P2-016 |
| Identity, auth & health | App Service Easy Auth + MCP JWT/JWKS env | `WEBSITE_AUTH_AAD_ALLOWED_TENANTS` single-tenant; `MCP_JWKS_URI` / `MCP_AUTH_ISSUER` / `MCP_AUTH_AUDIENCE` derived from one Easy Auth issuer; `ENABLE_AUTH=false` and `MCP_ENABLE_AUTH=false` defaults | Single-tenant config; MCP JWT validation is wired but disabled by default; no tenant-pinning for the frontend during failover | P0-026, P0-028, P1-010, P1-011, P3-014 |
| Identity, auth & health | `/healthz` middleware config | `HealthCheckMiddleware` registered with `checks={}` and an empty-string `password` literal | Always returns healthy regardless of dependency state; no GLB-grade readiness contract; secret is empty literal in code | P0-021, P0-025, P1-040, P3-010 |
| Identity, auth & health | Container env-var bindings | Backend Container App `env` block hardcodes `COSMOSDB_ENDPOINT`, `AZURE_AI_AGENT_ENDPOINT`, `AZURE_SEARCH_SERVICE_ENDPOINT`, `AZURE_STORAGE_ACCOUNT_NAME`, all derived from primary-region module outputs | Backend cannot reach a secondary-region data plane without a redeploy; no env-var indirection through GLB / runtime config | P0-027, P0-029, P1-017, P1-042 |
| Networking & operator access | Virtual Network + subnets | `infra/modules/virtualNetwork.bicep` creates VNet with `addressPrefixes: ['10.0.0.0/8']` and 5 hardcoded subnet CIDRs | `/8` consumes a private-IP-space block far in excess of need; subnet CIDRs are not parameterized; cannot peer cleanly with another `10/8` deployment | P2-014, P3-011, P3-012 |
| Networking & operator access | Bastion + Windows jumpbox | AVM bastion-host + virtual-machine; `virtualMachineAvailabilityZone = 1`; PPG attached; jumpbox admin password param marked `string?` with literal fallback in scripts | Bastion zone redundancy not configured; jumpbox pinned to AZ 1 + PPG; single-zone admin path; password literal in script when param empty | P1-008, P1-009, P1-015, P0-023 |
| Supply chain | Container Registry reference | `backendContainerRegistryHostname`, `frontendContainerRegistryHostname`, `MCPContainerRegistryHostname` all default to `biabcontainerreg.azurecr.io` | External Microsoft-operated ACR baked into Container Apps and Web App env vars; no customer-controlled ACR; mutable `latest_v4` tag | P0-022, P1-005, P1-012, P1-019, P1-038, P3-009 |
| Supply chain | AVM module references | `br/public:avm/...` pinned at minor versions across `main.bicep` | Cross-repo supply-chain dependency on the public AVM registry; module bumps require manual pinning review | P1-036 |
| Supply chain | Operator post-deploy scripts | `infra/scripts/Selecting-Team-Config-And-Data.ps1` runs `az storage blob upload-batch --overwrite` against the deployed Storage account; agent metadata seeding script writes Foundry agent IDs | Cross-region deployment requires the operator to re-run these scripts against the secondary stack; `--overwrite` has no concurrency control and `latest_v4` images are pulled at run time | P2-009, P3-007 |
| Supply chain | GitHub Actions workflows | Workflows under `.github/workflows/` reference third-party actions by tag, not commit SHA | Tag-pinned third-party actions can be re-pointed by their owners; no SHA pinning enforces an immutable supply chain | P2-013 |

## Not Available (External Repos / Platform / Operator)

| Capability | Owner | Needed for | Findings |
| --- | --- | --- | --- |
| Global Load Balancer / Front Door / Traffic Manager profile | Platform | Failover decision-maker between `{primaryRegion}` and `{secondaryRegion}`; required for the readiness contract that `/healthz` is meant to feed | P0-021, P0-024 |
| Customer-owned Azure Container Registry with geo-replication | Platform | Replacement for the external `biabcontainerreg.azurecr.io` so image pulls survive cross-region failover and SHA-pinned cold-starts | P0-022, P1-005, P1-012, P1-019 |
| Azure Key Vault | Platform | Source for the jumpbox admin password and any other secret currently inlined in scripts or env defaults; required to remove the empty-string `password` literal in `HealthCheckMiddleware` | P0-023, P3-010 |
| Cosmos DB provisioned-throughput migration target | Operator | Serverless Cosmos cannot be promoted to multi-region writes in place; a migration to provisioned RU/s is a prerequisite for any multi-region topology | P0-013, P1-023 |
| Cosmos multi-region read / write topology + conflict-resolution policy | Platform | `locations[]` array, `enableMultipleWriteLocations`, and a `conflictResolutionPolicy` are required to enable geo-replication on the Cosmos account | P0-003, P1-018, P1-021, P1-023, P2-004 |
| ACR geo-replication configuration + digest-pinned image references | Platform | Required for cold-start in `{secondaryRegion}` and for an immutable supply chain (current refs use mutable `latest_v4`) | P1-005, P3-009 |
| Per-region parameter sets / `primaryLocation` + `secondaryLocation` Bicep parameters | External repo | Replacement for the single `location` parameter and the hardcoded `cosmosDbZoneRedundantHaRegionPairs` and `replicaRegionPairs` tables | P0-005, P1-013, P1-014, P1-017 |
| Cross-region DNS records / FQDN aliasing | Operator | Required for the GLB to address per-region origins for the Web App, Container App, and MCP server | P0-024, P1-040 |
| Cosmos Private Endpoint in `{secondaryRegion}` and Private DNS zone link to both VNets | Platform | Required to keep Cosmos traffic on private endpoints during failover; without it the secondary VNet cannot resolve the Cosmos account privately | P0-030, P1-029 |
| Operator runbooks for failover, RBAC propagation readiness, and Foundry agent metadata reseeding | Operator | Required during cutover; the repo has no readiness gate for Entra RBAC propagation and no reseeding flow for agent IDs | P1-007, P1-039, P2-016 |
| Azure Monitor / Log Analytics replica region operator override | Operator | Required to align telemetry residency with the chosen secondary region; the replica region is hardcoded in `replicaRegionPairs` | P1-014, P2-002 |
| Continuous backup / PITR posture for Cosmos and blob versioning + restore policy + change feed for Storage | Platform | Required for an explicit RPO contract; serverless Cosmos blocks PITR and the Storage account has no versioning / restore policy | P2-007, P2-008 |
| Service Bus / Web PubSub / Redis Pub-Sub backplane | External repo | Cross-replica WebSocket and orchestration backplane to replace the in-memory connection map, in-memory checkpoint storage, and `asyncio.Event` pre-approval state | P0-008, P0-009, P0-010, P0-011, P0-018, P0-019, P0-020, P1-027, P1-028 |
| Azure Bastion peer + jumpbox peer in `{secondaryRegion}` | Platform | Operator access path during a primary-region outage; current bastion + jumpbox are single-region and AZ-1-pinned | P1-008, P1-009, P1-015 |
